...
class TextMiner:
    ...
    def __init__(self,
        ...
        self.lang: str = 'en' if lang is None else lang  #None
        ...
        for feature in self.features:
            self.text_features_len.update({feature: list(self.df[feature].fillna('').apply(func=len_of_str).values)})
    ...
    def _apply_clustering(self, features: List[str], k: int = 3):
        ...
        for feature in features:
            ...
            _clean_text_data: pd.DataFrame = self.df[feature].apply(lambda x: self.clean_text(phrase=str(x)), meta=(feature, 'object'))
            ...
            _text_clustering: dict = UnsupervisedML(df=_tfidf_matrix,
                                                    n_cluster_components=k
                                                    ).run_clustering(cluster_algorithms=['kmeans'])
            ...
            if feature in self.df.columns:
                ...
                for x, case in enumerate(self.df[feature].values):
                    ...
                    if isinstance(case, str):
                        ...
                        _nlp = _model(self.clean_text(phrase=str(case)))
                        for other_case in self.df[feature].values:
                            if case != other_case:
                                self.similarity_scores[feature][x].append(self.lang_models[self.lang_model_framework][_lang]['model'](self.clean_text(phrase=str(other_case))))
    ...
    def _extract_email(self, feature: str = None):
        ...
        for ft in _features:
            self.web.update({ft: dict(email=[])})
            for case in self.df[ft].to_dask_array(lengths=True).compute():
                self.web[ft]['email'].append(str(case).split('@')[1])
    ...
    def _extract_url(self, feature: str = None):
        ...
        for ft in _features:
            self.web.update({ft: dict(url=dict(domain=[], domain_ext=[]))})
            for case in self.df[ft].to_dask_array(lengths=True).compute():
                self.web[ft]['url']['domain'].append(str(case).split('.')[1] if str(case).find('www.') >= 0 else str(case).split('.')[0])
                self.web[ft]['url']['domain_ext'].append(str(case).split('.')[-1].split('/')[0] if str(case).find('www.') >= 0 else str(case).split('.')[-1].split('/')[0])
    ...
    def _get_lang(self):
        ...
        for _ in range(0, 10, 1):
            _text_samples.append(np.random.choice(a=self.df[self.text_feature].values.tolist()))
        ...
        if len(_lang_each_case) > 0:
            self.detected_language.update({self.text_feature: dict(unique=_lang_each_case,
                                                                   most_freq=collections.Counter(_lang_each_case).most_common(1)[0][0]
                                                                   )
                                           })
    ...
    def _is_entity(self) -> bool:
        ...
        _df['len'] = np.array(self.text_features_len.get(_feature))
        _df = _df.fillna('')
        if len(_df) == 0:
            return False
    ...
    def _is_phrase(self) -> bool:
        ...
        _signals: int = 0
        _feature: str = copy.deepcopy(self.text_feature)
        _df: pd.DataFrame = pd.DataFrame({_feature: [_ for _ in range(0, len(self.df), 1)]})
        ...
        for feature in self.features:
            ...
            if phrases:
                if self._is_phrase():
                    ...
                    continue
            if identifier:
                if len(self.df) == len(self.df[feature].unique().compute()):
    ...
    def clean_text(self,
        ...
        _lang: str = 'en' if self.lang is None else self.lang
    ...
    def get_all_generated_features(self, features: List[str] = None, compute: bool = False):
        """
    ...
    def get_features(self, features: List[str], meth: str = None, compute: bool = False):
        """
        Get generated features as Pandas or dask DataFrame

        :param features: List[str]
            Name of the features

        :param meth: str
            Name of the engineering method
        ...
        for feature in features:
            if feature in self.generated_features.keys():
                if meth is None:
                    for method in self.generated_features[feature].keys():
                        _processed_features.extend(self.generated_features[feature][method])
                else:
                    if meth in self.generated_features[feature].keys():
                        _processed_features.extend(self.generated_features[feature][meth])
        if len(_processed_features) == 0:
            Log(write=False, level='info').log(msg='No generated features found')
        if compute:
            return self.df[_processed_features].compute()
        else:
            return self.df[_processed_features]
    ...
    def generate_categorical_features(self, features: List[str] = None):
        ...
        for cat in ['email', 'url']:
            if self.segments.get(cat) is not None:
                _categorical_features.extend(self.segments.get(cat))
        _features: List[str] = _categorical_features if features is None else [feature for feature in features if feature in _categorical_features]
        for feature in _features:
    ...
    def generate_linguistic_features(self, features: List[str] = None):
        ...
        for feature in _features:
            if feature in self.df.columns:
                ...
                self.df['{}_ner_entity'.format(feature)] = np.array(self.ner[feature]['entity'])
                self.df['{}_ner_entity_type'.format(feature)] = np.array(self.ner[feature]['entity_type'])
                #self.df['{}_dep_tree_children'.format(feature)] = np.array(self.dep[feature]['tree']['children'])
                #self.df['{}_count_special_chars'.format(feature)] = np.array(self.special_chars[feature]['count'])
                #self.df['{}_count_unique_special_chars'.format(feature)] = np.array(self.special_chars[feature]['count_unique'])
                self.df['{}_clean_length'.format(feature)] = np.array(self.clean_phrases[feature]['count_len'])
                self.df['{}_clean_count_words'.format(feature)] = np.array(self.clean_phrases[feature]['count_words'])
                self.df['{}_clean_count_sents'.format(feature)] = np.array(self.clean_phrases[feature]['count_sents'])
                self.df['{}_clean_count_conjuncts'.format(feature)] = np.array(self.clean_phrases[feature]['count_conjuncts'])
                self.df['{}_pos_count_unique_tags'.format(feature)] = np.array(self.pos[feature]['count_unique_tags'])
                self.df['{}_pos_count_unique_labels'.format(feature)] = np.array(self.pos[feature]['count_unique_labels'])
                self.df['{}_pos_count_ad'.format(feature)] = np.array(self.pos[feature]['count_ad'])
                self.df['{}_pos_count_verb'.format(feature)] = np.array(self.pos[feature]['count_verb'])
                self.df['{}_pos_count_noun'.format(feature)] = np.array(self.pos[feature]['count_noun'])
                self.df['{}_ner_recognized'.format(feature)] = np.array(self.ner[feature]['recognized'])
                self.df['{}_ner_count'.format(feature)] = np.array(self.ner[feature]['count'])
                self.df['{}_ner_count_unique'.format(feature)] = np.array(self.ner[feature]['count_unique'])
                #self.df['{}_dep_tree_count_has_child'.format(feature)] = np.array(self.dep[feature]['tree']['count_has_child'])
                self.df['{}_dep_tree_count_dep_of_child'.format(feature)] = np.array(self.dep[feature]['tree']['count_dep_of_child'])
                self.df['{}_dep_tree_count_unique_dep_of_child'.format(feature)] = np.array(self.dep[feature]['tree']['count_unique_dep_of_child'])
                self.df['{}_dep_tree_count_head_text_of_child'.format(feature)] = np.array(self.dep[feature]['tree']['count_head_text_of_child'])
                self.df['{}_dep_tree_count_unique_head_text_of_child'.format(feature)] = np.array(self.dep[feature]['tree']['count_unique_head_text_of_child'])
                self.df['{}_dep_tree_count_head_pos_of_child'.format(feature)] = np.array(self.dep[feature]['tree']['count_head_pos_of_child'])
                self.df['{}_dep_tree_count_unique_head_pos_of_child'.format(feature)] = np.array(self.dep[feature]['tree']['count_unique_head_pos_of_child'])
                #self.df['{}_dep_tree_count_children'.format(feature)] = np.array(self.dep[feature]['tree']['count_children'])
                self.df['{}_dep_noun_pairs_count_children'.format(feature)] = np.array(self.dep[feature]['noun_pairs']['count_dep'])
                self.df['{}_dep_noun_pairs_count_unique_dep'.format(feature)] = np.array(self.dep[feature]['noun_pairs']['count_unique_dep'])
                self.df['{}_dep_noun_pairs_count_root_dep'.format(feature)] = np.array(self.dep[feature]['noun_pairs']['count_root_dep'])
                self.df['{}_dep_noun_pairs_count_unique_root_dep'.format(feature)] = np.array(self.dep[feature]['noun_pairs']['count_unique_root_dep'])
                self.df['{}_dep_noun_pairs_count_root_head_dep'.format(feature)] = np.array(self.dep[feature]['noun_pairs']['count_root_head_dep'])
                self.df['{}_dep_noun_pairs_count_unique_root_head_dep'.format(feature)] = np.array(self.dep[feature]['noun_pairs']['count_unique_root_head_dep'])
                self.df['{}_dep_noun_pairs_count_root_text_dep'.format(feature)] = np.array(self.dep[feature]['noun_pairs']['count_root_text_dep'])
                self.df['{}_dep_noun_pairs_count_unique_root_text_dep'.format(feature)] = np.array(self.dep[feature]['noun_pairs']['count_unique_root_text_dep'])
                #self.df['{}_has_emojis'.format(feature)] = np.array(self.emoji[feature]['has'])
                #self.df['{}_emojis_count'.format(feature)] = np.array(self.emoji[feature]['count'])
                #self.df['{}_emojis_count_unique'.format(feature)] = np.array(self.emoji[feature]['count_unique'])
                #self.df['{}_emojis_label'.format(feature)] = np.array(self.emoji[feature]['label'])
    ...
    def merge(self, features: List[str], sep: str = ' '):
        ...
        _pairs: List[tuple] = get_pairs(features=_features, max_features_each_pair=2)
        for pair in _pairs:
    ...
    def similarity(self, features: List[str] = None):
        ...
        for feature in self.similarity_scores.keys():
            ...
            self.df[self.generated_features[feature]['similarity'][-1]] = np.array(_score_each_case)
    ...
    def splitter(self, features: List[str] = None, sep: str = None):
        ...
        """
        _features: List[str] = self.segments.get('enumeration') if features is None else [feature for feature in features if feature in self.df.columns]
        for feature in _features:
            if feature in self.df.columns:
                if self.generated_features.get(feature) is None:
                    self.generated_features.update({feature: dict(split=[])})
                ...
                if _sep is None:
                    ...
                    _df: pd.DataFrame = pd.DataFrame()
                    _df[feature] = self.df[feature].compute()
                    _split_features: pd.DataFrame = _df[feature].str.split(pat=_sep, expand=True)
                    if _split_features.shape[1] <= 1:
                        Log(write=False, level='info').log(msg='Separator ({}) found in text feature "{}"'.format(_sep, feature))
                    else:
                        _split_features = _split_features.astype(dtype=str)
                        ...
                        print(_split_features)
                        print(_split_features.dtypes)
                        for sf in _split_features.keys():
                            self.generated_features[feature]['split'].append(sf)
                            self.df[sf] = _split_features[sf].values
    ...
    def tfidf(self, features: List[str] = None):
        ...
        for feature in _features:
            if feature in self.df.columns:
                ...
                _clean_text_data: np.array = np.array([self.clean_text(phrase=str(phrase)) for phrase in self.df[feature].values])
                self.generated_features[feature]['tfidf'].append('{}_tfidf'.format(feature))
                self.df[self.generated_features[feature]['tfidf'][-1]] = _tfidf_vectorizer.fit_transform(_clean_text_data).data[0:self.df.shape[0]]
    ...
    def translate(self, text: str, lang: str) -> str:
        ...
        :return: str
            Translated text
        """
        raise NotImplemented('Translation not supported')
