...
from supervised_machine_learning import Classification, Regression
...
class FeatureSelector:
    ...
    def __init__(self,
        ...
        :param model_param: dict
            Pre-defined model hyperparameter
        """
        ...
        self.model_param: dict = model_param
        ...
        if self.ml_type == 'reg':
            self.model: Regression = Regression(reg_params=model_param, model_name=model_name)
        else:
            self.model: Classification = Classification(clf_params=model_param, model_name=model_name)
        self.model.generate()
        ...
        self.feature_selection_early_stopping: bool = feature_selection_early_stopping
    ...
    def _feature_addition(self, imp_features: List[str]) -> dict:
        ...
        :return: dict
            Redundant features, important features, gaining values, model metrics, base metric, threshold
        """
        Log().log(msg='Apply feature addition algorithm for feature selection based on calculated feature importance score')
        _features: List[str] = copy.deepcopy(imp_features[0:2])
        _model: Union[Classification, Regression] = copy.deepcopy(self.model)
        _model.train(x=self.train_df[_features].values, y=self.train_df[self.target_feature].values)
        _pred = _model.predict(x=self.test_df[_features].values)
        _model_test_score: float = self.model.eval(obs=self.test_df[self.target_feature].values, pred=_pred)
        Log().log(msg=f'Base model metric: {_model_test_score} - Features=2')
        if self.ml_type == 'reg':
            _threshold: float = _model_test_score - (_model_test_score * self.redundant_threshold)
        else:
            _threshold: float = _model_test_score + (_model_test_score * self.redundant_threshold)
        Log().log(msg=f'Metric threshold: {_threshold}')
        _result: dict = dict(redundant=[],
                             important=imp_features[0:2],
                             gain={},
                             model_metric=[],
                             base_metric=_model_test_score,
                             threshold=_threshold
                             )
        Log().log(msg=f'Two most important features: {_features}')
        for i in range(2, len(imp_features) - 1, 1):
            _features.append(imp_features[i])
            Log().log(msg=f'Add feature: {imp_features[i]}')
            _model: Union[Classification, Regression] = copy.deepcopy(self.model)
            _model.train(x=self.train_df[_features].values, y=self.train_df[self.target_feature].values)
            _pred = _model.predict(x=self.test_df[_features].values)
            _new_model_test_score: float = _model.eval(obs=self.test_df[self.target_feature].values, pred=_pred)
            Log().log(msg=f'New model metric: {_new_model_test_score} (Metric threshold: {_threshold}) - Features={i + 1}')
            if self.ml_type == 'reg':
                if _threshold <= _new_model_test_score:
                    _result['threshold'] = _threshold
                    if self.feature_selection_early_stopping:
                        _result['redundant'].extend(imp_features[i + 1:len(imp_features)])
                        break
                    else:
                        _result['redundant'].append(imp_features[i])
                        del _features[_features.index(imp_features[i])]
                else:
                    _threshold: float = _new_model_test_score - (_new_model_test_score * self.redundant_threshold)
                    Log().log(msg=f'New metric threshold: {_threshold}')
                    _result['important'].append(imp_features[i])
    ...
    def _filter_based(self, imp_features: List[str]) -> dict:
        ...
        Log().log(msg=f'Filter top {_top_n_imp_features} important features')
        return dict(redundant=imp_features[_top_n_imp_features:len(imp_features)], important=imp_features[0:_top_n_imp_features + 1])
    ...
    def _game(self, iteration: int):
        ...
        for pair in self.pairs:
            _game: Union[Classification, Regression] = copy.deepcopy(self.model)
            ...
            if self.ml_type == 'reg':
                ...
                _model_train_score: float = _game.eval(obs=self.train_df[self.target_feature].values, pred=_train_pred)
                _model_test_score: float = _game.eval(obs=self.test_df[self.target_feature].values, pred=_test_pred)
    ...
    def _recursive_feature_elimination(self, imp_features: List[str]) -> dict:
        ...
        _imp_features: List[str] = copy.deepcopy(imp_features)
        _imp_features.reverse()
        _model: Union[Classification, Regression] = copy.deepcopy(self.model)
        _model.train(x=self.train_df[imp_features].values, y=self.train_df[self.target_feature].values)
        _pred = _model.predict(x=self.test_df[imp_features].values)
        _model_test_score: float = _model.eval(obs=self.test_df[self.target_feature].values, pred=_pred)
        Log().log(msg=f'Base model metric: {_model_test_score} - Features={len(imp_features)}')
        if self.ml_type == 'reg':
        ...
        for feature in _imp_features:
            del _features[_features.index(feature)]
            if len(_features) == 1:
                _result['important'] = _features
                break
            Log().log(msg=f'Remove feature: {feature}')
            _model: Union[Classification, Regression] = copy.deepcopy(self.model)
            _model.train(x=self.train_df[_features].values, y=self.train_df[self.target_feature].values)
            _pred = _model.predict(x=self.test_df[_features].values)
            _new_model_test_score: float = _model.eval(obs=self.test_df[self.target_feature].values, pred=_pred)
            Log().log(msg=f'New model metric: {_new_model_test_score} (Metric threshold: {_threshold}) - Features={len(_features)}')
            if self.ml_type == 'reg':
                if _threshold <= _new_model_test_score:
                    ...
                    if self.feature_selection_early_stopping:
                        ...
                        _result['important'].append(feature)
                else:
                    _result['redundant'].append(feature)
                    _result['model_metric'].append(_model_test_score)
                    _result['reduction'].update({feature: _model_test_score - _new_model_test_score})
            else:
                if _threshold >= _new_model_test_score:
                    if self.feature_selection_early_stopping:
                        _features.append(feature)
    ...
    def main(self,
        ...
        for r, val in enumerate(_sorted_scores):
            if r == 0:
                ...
                if val == _sorted_scores[r - 1]:
                    ...
                    _rank.append(r + 1)
        _df['rank'] = _rank
        self.plot.update({'Feature Importance (Shapley Scores)': dict(df=_df.to_dict(),
                                                                      plot_type='bar',
                                                                      render=False,
                                                                      file_path=os.path.join(plot_path, 'feature_importance_shapley.html'),
                                                                      kwargs=dict(layout={},
                                                                                  y=_df['score'].values.tolist(),
                                                                                  x=_df.index.values.tolist(),
                                                                                  marker=dict(color=_df['score'].values.tolist(),
                                                                                              colorscale='rdylgn',
                                                                                              autocolorscale=True
                                                                                              )
                                                                                  )
        ...
        self.plot.update({'Feature Tournament Game Stats (Shapley Scores)': dict(df=_game_df.to_dict(),
                                                                                 features=_game_df.columns.tolist(),
                                                                                 plot_type='violin',
                                                                                 melt=True,
                                                                                 render=False,
                                                                                 file_path=os.path.join(plot_path, 'feature_tournament_game_stats.html'),
                                                                                 kwargs=dict(layout={})
                                                                                 )
                          })
        ...
        self.plot.update({'Feature Tournament Stats (Game Size)': dict(df=_tournament_df.to_dict(),
                                                                       features=_tournament_df.columns.tolist(),
                                                                       plot_type='heat',
                                                                       render=False,
                                                                       file_path=os.path.join(plot_path, 'feature_tournament_game_size.html'),
                                                                       kwargs=dict(layout={})
                                                                       )
                          })
        if aggregate_feature_imp is not None:
            ...
            for core_feature in aggregate_feature_imp.keys():
                ...
                self.plot.update({f'Feature Importance (Preprocessing Variants {core_feature})': dict(data=_processed_feature_matrix.to_dict(),
                                                                                                      plot_type='bar',
                                                                                                      melt=True,
                                                                                                      render=False,
                                                                                                      file_path=os.path.join(plot_path, 'feature_importance_processing_variants.html'),
                                                                                                      kwargs=dict(layout={},
                                                                                                                  y=_processed_feature_matrix['score'].values.tolist(),
                                                                                                                  x=_processed_feature_matrix.index.values.tolist(),
                                                                                                                  marker=dict(color=_processed_feature_matrix['score'].values.tolist(),
                                                                                                                              colorscale='rdylgn',
                                                                                                                              autocolorscale=True
                                                                                                                              )
                                                                                                                  )
            ...
            self.plot.update({'Feature Importance (Core Feature Aggregation)': dict(data=_core_imp_matrix.to_dict(),
                                                                                    plot_type='bar',
                                                                                    melt=False,
                                                                                    render=False,
                                                                                    file_path=os.path.join(plot_path, 'feature_importance_core_aggregation.html'),
                                                                                    kwargs=dict(layout={},
                                                                                                y=_core_imp_matrix['abs_score'].values.tolist(),
                                                                                                x=_core_imp_matrix['abs_score'].index.values.tolist(),
                                                                                                marker=dict(color=_core_imp_matrix['abs_score'].values.tolist(),
                                                                                                            colorscale='rdylgn',
                                                                                                            autocolorscale=True
                                                                                                            )
                                                                                                )
        ...
        Log().log(msg=f'Number of important features: {len(_feature_selection_result["important"])}')
