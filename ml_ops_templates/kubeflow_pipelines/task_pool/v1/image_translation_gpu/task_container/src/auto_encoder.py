...
class AutoEncoder:
    ...
    def _build_auto_encoder_network(self):
        ...
        # Encoder:
        _encoder: tf.Tensor = Conv2D(filters=_n_filters,
                                     kernel_size=(3, 3),
                                     strides=(1, 1),
                                     padding='same',
        ...
        # Decoder:
        _decoder: tf.Tensor = Conv2D(filters=_n_filters,
                                     kernel_size=(3, 3),
                                     strides=(1, 1),
                                     padding='same',
                                     kernel_initializer=self.initializer
    ...
    def _save_models(self, model_output_path: str):
        ...
        """
        _file_path: str = model_output_path
        _file_name: str = 'image_translator_auto_encoder.h5'
        self.model.save(filepath=_file_name)
        save_file_to_s3(file_path=os.path.join(_file_path, _file_name), obj=None, input_file_path=_file_name)
