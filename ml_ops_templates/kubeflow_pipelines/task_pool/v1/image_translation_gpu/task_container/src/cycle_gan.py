...
class CycleGAN:
    ...
    def _build_discriminator(self) -> Model:
        ...
        _d: tf.Tensor = Conv2D(filters=_n_filters,
                               kernel_size=(4, 4),
                               strides=(2, 2),
                               padding='same',
        ...
        _d = LeakyReLU(alpha=0.2)(_d)
        _d = ZeroPadding2D(padding=(1, 1))(_d)
        _patch_out: tf.Tensor = Conv2D(filters=self.n_channels,
                                       kernel_size=(4, 4),
                                       strides=(1, 1),
                                       padding='valid',
                                       kernel_initializer=self.initializer,
    ...
    def _convolutional_layer_discriminator(self, input_layer, n_filters: int) -> tf.Tensor:
        """
        ...
        """
        _d: tf.Tensor = Conv2D(filters=n_filters,
                               kernel_size=(4, 4),
                               strides=(2, 2),
                               padding='same',
                               kernel_initializer=self.initializer
    ...
    def _convolutional_layer_generator_decoder(self,
        ...
        :param input_layer: tf.Tensor
            Network layer to process in the first convolutional layer

        :param skip_layer: tf.Tensor
        ...
        """
        if self.generator_type == 'u':
            _u: tf.Tensor = UpSampling2D(size=(2, 2), interpolation='bilinear')(input_layer)
    ...
    def _residual_network(self):
        ...
        _g: tf.Tensor = ReflectionPadding2D(padding=(3, 3))(_input)
        ...
        _fake_image: tf.Tensor = Conv2D(filters=self.n_channels,
                                        kernel_size=(7, 7),
                                        strides=(1, 1),
                                        padding='valid',
                                        kernel_initializer=self.initializer,
    ...
    def _residual_network_block(self,
        ...
        :return: tf.Tensor
            Processed tensorflow tensor
        """
        _r: tf.Tensor = ReflectionPadding2D(padding=(1, 1))(input_layer)
    ...
    def _u_network(self) -> Model:
        ...
        _input: Input = Input(shape=self.image_shape)
        _n_filters: int = self.start_n_filters_generator
        _g: tf.Tensor = Conv2D(filters=_n_filters,
                               kernel_size=(3, 3),
                               strides=(1, 1),
                               padding='same',
                               kernel_initializer=self.initializer
    ...
    def main(self,
        ...
        :param multi_gpu: bool
            Whether to train cycle-gan using multi-gpu distribution or not
        """
        Log().log(msg=f'Physical GPU devices: {tf.config.list_physical_devices("GPU")}')
        if multi_gpu:
            # Multi-GPU distributed training:
            _strategy = tf.distribute.MirroredStrategy()
            Log().log(msg=f'Number of devices to use: {_strategy.num_replicas_in_sync}')
            with _strategy.scope():
                self.train(model_output_path=model_output_path,
                           ...
                           random_noise_types=random_noise_types,
