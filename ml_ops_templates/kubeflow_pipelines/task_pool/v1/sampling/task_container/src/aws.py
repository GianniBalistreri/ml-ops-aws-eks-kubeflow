...
def get_file_from_s3(s3_bucket_name: str) -> List[str]:
    """
    ...
    paginator: boto3.client.S3.ListObjectsV2 = _s3_client.get_paginator('list_objects_v2')
    _s3_objects: list = []
    for page in paginator.paginate(Bucket=s3_bucket_name):
        if 'Contents' in page:
            _s3_objects.extend(page['Contents'])
    _file_names: List[str] = []
    for obj in _s3_objects:
        _file_names.append(obj['Key'])
    return _file_names
