...
from aws import get_file_from_s3
...
class FileSamplerException(Exception):
    """
    Class for handling exceptions for class FileSampler
    """
    pass
...
class FileSampler:
    """
    Class for sampling files
    """
    def __init__(self, file_path: str, size: int = None, prop: float = None):
        """
        :param file_path: str
            File path
        ...
        self.file_names: List[str] = get_file_from_s3(s3_bucket_name=file_path)
        if size is None:
            if prop is None:
                raise SamplerException('Neither size nor proportion found')
            self.k: int = round(len(self.file_names) * prop)
        else:
            self.k: int = size

    def main(self, meth: str = 'random') -> List[str]:
        """
        File sampling

        :param meth: str
            File sampling method
        ...
        """
        if meth == 'random':
            return random.sample(population=self.file_names, k=self.k)
        elif meth == 'first':
            return self.file_names[0:self.k]
        elif meth == 'last':
            return self.file_names[len(self.file_names) - self.k:len(self.file_names)]
        else:
            raise FileSamplerException(f'File sampling method ({meth}) not supported')
...
class MLSampler:
    ...
    def down_sampling(self, target_class_value: Union[str, int], target_proportion: float) -> pd.DataFrame:
        ...
        if 0 < target_proportion < 1:
            ...
            _population: list = self.df.loc[self.df[self.target] == target_class_value, :].index.values.tolist()
            _sample_idx: list = random.sample(population=_population, k=_target_absolute_count)
    ...
    def up_sampling(self, target_class_value: Union[str, int], target_proportion: float) -> pd.DataFrame:
        """
        Up sample specific ranges of target values to reach a target proportion.
        ...
        Log().log(msg=f'Target feature: {self.target} -> target class value: {target_class_value} -> absolute frequency = {_current_count}, relative frequency = {_current_proportion}')
        if _current_proportion >= target_proportion:
            raise MLSamplerException(f'Given target proportion ({target_proportion}) is smaller than or equal to current proportion ({_current_proportion})')
        _target_absolute_count: int = round((target_proportion / _current_proportion) * self.df.shape[0]) - _current_count
        _sample_idx: list = random.sample(population=self.df.loc[self.df[self.target] == target_class_value, :].index.values.tolist(), k=_target_absolute_count)
        _population: list = self.df.loc[self.df[self.target] == target_class_value, :].index.values.tolist()
        _proportion: float = len(_population) / _target_absolute_count
        _fraction_of_population: float = float(f'0.{str(_proportion).split(".")[-1]}')
        _fraction_absolute_count: int = round(len(_population) * _fraction_of_population)
        _df: pd.DataFrame = self.df
        for s in range(0, int(_proportion), 1):
            _df: pd.DataFrame = pd.concat(objs=[_df, self.df.iloc[_population, :]], axis=0)
        _sample_idx: list = random.sample(population=_population, k=_fraction_absolute_count)
        _df: pd.DataFrame = pd.concat(objs=[self.df, self.df.iloc[_sample_idx, :]], axis=0)
        _new_count, _new_proportion = self._get_freq_of_target_class_value(df=_df,
                                                                           target_class_value=target_class_value
                                                                           )
        Log().log(msg=f'Up-sample target feature: {self.target} -> target class value: {target_class_value} -> new absolute frequency = {_new_count}, new relative frequency = {_new_proportion}, requested relative frequency = {target_proportion}')
        return _df
