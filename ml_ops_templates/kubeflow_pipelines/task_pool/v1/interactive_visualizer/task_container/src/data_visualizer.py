...
def generate_network(df: pd.DataFrame, node_feature: str, edge_feature: str, kind: str = 'undirected', **kwargs) -> nx:
    ...
    if kind == 'directed':
        ...
        raise DataVisualizerException('Network graph type ({}) not supported'.format(kind))
    if node_feature not in df.keys():
        raise DataVisualizerException('Node feature ({}) not found'.format(node_feature))
    if edge_feature not in df.keys():
        raise DataVisualizerException('Edge feature ({}) not found'.format(edge_feature))
...
def get_group_by_percentile(data: pd.DataFrame,
    ...
    """
    Generate percentile evaluation of grouped features
    ...
    if group_by in data.keys():
        ...
        if len(aggregate_by) == 0:
            raise DataVisualizerException('No features for aggregation found')
        for agg in aggregate_by:
            if agg in data.keys():
                _aggre.append(agg)
                _df[agg] = data[agg]
    else:
        raise DataVisualizerException('No feature for grouping found')
    if include_group:
...
def load_plotly_from_json(file_path: str, **kwargs):
    """
    ...
    """
    _fig: dict = load_file_from_s3(file_path=file_path)
...
class DataVisualizer:
                ...
                for plot in self.subplots.keys():
                    if isinstance(self.subplots.get(plot), dict):
                        if 'data' in self.subplots.get(plot).keys():
                            if isinstance(self.subplots[plot].get('data'), dict):
                                self.subplots[plot]['data'] = pd.DataFrame(data=self.subplots[plot]['data'])
                        else:
                            if self.df is None:
                                if 'df' in self.subplots.get(plot).keys():
                                    if isinstance(self.subplots[plot].get('df'), dict):
                                        self.subplots[plot].update({'data': pd.DataFrame(data=self.subplots[plot]['df'])})
                                    else:
                                        self.subplots[plot].update({'data': self.subplots[plot]['df']})
                        ...
                        raise DataVisualizerException('Subplots dictionary should contain dictionaries')
            else:
                raise DataVisualizerException('Subplots parameter should be a dictionary containing dictionaries -> Dict[str, dict]')
    ...
    def _plotly_histogram_decile_chart(self, color_scale: List[tuple], color_feature: np.array):
        ...
        """
        for group in self.plot.get('group_by'):
            _perc_table: pd.DataFrame = get_group_by_percentile(data=self.df,
                                                                group_by=group,
            ...
            _min_table: pd.DataFrame = get_group_by_percentile(data=self.df,
                                                               group_by=group,
                                                               aggregate_by=self.plot.get(
                                                                   'features'),
            ...
            _max_table: pd.DataFrame = get_group_by_percentile(data=self.df,
                                                               group_by=group,
                                                               aggregate_by=self.plot.get(
                                                                   'features'),
    ...
    def _plotly_network_graph_chart(self, color_scale: List[tuple], color_feature: np.array):
        ...
        _graph: nx = generate_network(df=self.df,
    ...
    def _plotly_parallel_category_chart(self):
        ...
        if self.brushing and len(_brushing) > 0:
            if len(_brushing) == 1:
                ...
                _pairs: List[tuple] = get_pairs(features=_brushing, max_features_each_pair=2)
                for pair in _pairs:
        ...
        if self.plot.get('group_by') is None:
            ...
            for i, ft in enumerate(self.plot.get('features')):
                if _desc.get(ft) is None:
                    ...
                    self.df[ft] = label_encoder(values=self.df[ft].values)
    ...
    def _plotly_scatter_3d_chart(self, color_scale: List[tuple], color_feature: np.array):
        ...
        _pairs: List[tuple] = get_pairs(features=_features, max_features_each_pair=3)
        if self.plot.get('group_by') is None:
            for i, pair in enumerate(_pairs, start=1):
                self.plot['kwargs'].update({'x': self.df[pair[0]].values,
                                            'y': self.df[pair[1]].values,
    ...
    def _plotly_sunburst_tree_chart(self):
        ...
        for conti in self.features:
            ...
            if len(_elements) >= 100:
                ...
                _pairs: List[tuple] = get_pairs(features=self.group_by, max_features_each_pair=2)
                for pair in _pairs:
    ...
    def _run_plotly_offline(self):
        ...
        for t, title in enumerate(self.subplots.keys()):
            ...
            if self.plot.get('yaxis_label') is not None:
                self.plot['kwargs']['layout'].update({'yaxis': dict(title=dict(text=self.plot.get('yaxis_label')))})
            if self.plot.get('color_feature') is not None:
                if str(self.df[self.plot.get('color_feature')].dtype).find('object') >= 0:
                    _color_feature = label_encoder(values=self.df[self.plot.get('color_feature')].values)
        ...
        if not self.get_fig:
            ...
            if self.plot.get('file_path') is not None:
                ...
                if len(self.plot.get('file_path')) > 0:
                    ...
                    self.file_paths.append(copy.deepcopy(self.plot.get('file_path')))
                    Log(write=False).log(f'Saving plotly chart: {self.plot.get("file_path")}')
