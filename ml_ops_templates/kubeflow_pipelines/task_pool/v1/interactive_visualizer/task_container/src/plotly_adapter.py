...
Wrapper for generating interactive visualizations using Plot.ly
...
class PlotlyAdapter:
    ...
    def _write_plotly_json(self):
        ...
        _fig: dict = dict(data=_data, layout=_layout)
        save_file_to_s3(file_path=self.plot.get('file_path'), obj=_fig)
    ...
    def load(self):
        ...
        """
        _fig: dict = load_file_from_s3(file_path=self.plot['file_path'])
    ...
    def save(self):
        ...
        if self.plot.get('file_path').split('.')[-1] == 'json':
        ...
        elif self.plot.get('file_path').split('.')[-1] in ['jpg', 'jpeg', 'png']:
            save_file_to_s3(file_path=self.plot.get('file_path'), obj=self.fig, plotly=True)
        elif self.plot.get('file_path').split('.')[-1] == 'html':
            if self.cloud is None:
                _ = plot(figure_or_data=go.FigureWidget(self.fig),
                         show_link=False if self.plot['kwargs'].get('show_link') is None else self.plot['kwargs'].get('show_link'),
                         link_text='Export to plot.ly' if self.plot['kwargs'].get('link_text') is None else self.plot['kwargs'].get('link_text'),
                         validate=True if self.plot['kwargs'].get('validate') is None else self.plot['kwargs'].get('validate'),
                         image=self.plot['kwargs'].get('image'),
            ...
            Log().log(f'File format ({self.plot.get("file_path").split(".")[-1]}) not supported for saving plotly charts')
